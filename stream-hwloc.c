...
#pragma omp atomic 
  ...
  /* set interleave as the memory affinity binding policy */
  err = hwloc_set_membind(topology, cset,  HWLOC_MEMBIND_INTERLEAVE, HWLOC_MEMBIND_MIGRATE);	
  if (err < 0) {
    fprintf(stderr, "failed to set next interleave policy \n");
    hwloc_topology_destroy(topology);
    return EXIT_FAILURE;
  }
